...
const renderKawashima = () => {
  document.getElementById('app').innerHTML = `
    ...
    <main>
      ...
      <section class="kaw-products">
        ...
        <div class="kaw-prod-grid">
          ${[
            ...
            { sku: 'AK5L',       name: 'Aspersor manual 5L',                tag: 'Aspersión', spec: 'Manual / 5 litros<br>Compacto y fácil de operar',                                                    imgs: [imgAK5L_A] },
      ...
      ${buildNewProductsHTML('kaw', '#CC1111', 'KAWASHIMA', brandNewProds.kawashima.map(p => {
  ...
  const buildProdCard = (prod, isNew = false) => {
    ...
    return `
      <div class="${p}-prod-card" style="position:relative;">
        ...
        <div class="prod-illustration-wrap${prod.imgs ? ' has-imgs' : ''}" style="height:220px;${prod.imgs ? 'background:#f0f0ee;' : `background:rgba(0,0,0,0.25);border:1px solid ${cfg.accentColor}20;display:flex;align-items:center;justify-content:center;padding:20px;`}border-radius:14px;margin-bottom:18px;position:relative;overflow:hidden;transition:all 0.4s ease;">
          ${prod.imgs
